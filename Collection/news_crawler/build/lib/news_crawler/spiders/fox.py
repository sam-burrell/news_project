...
class foxSpider(CrawlSpider):
    ...
    def parse_article(self, response):
        ...
        article.nested_css('meta[name="dcterms.created"]').add_xpath(
            'published_time',
            './@content',
            time_in,
        )
        article.add_xpath('category', '//head/meta[@name="classification"]/@content', lambda v: v[0].split('/') if v else None)
        #article.add_xpath('keywords', '//head/meta[@name="keywords"]/@content')
        article.add_value('encoding', response.encoding)
        article.nested_css('div.article-text').add_xpath('body', './p//text()')
